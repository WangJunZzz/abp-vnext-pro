...
import { AUHT_KEY } from '/@/enums/cacheEnum';
...
export const usePermissionStore = defineStore({
  ...
  getters: {
    getPermCodeList() {
      return this.permCodeList.length > 0 ? this.permCodeList : getAuthCache<string[]>(AUHT_KEY);
    },
    getBackMenuList() {
  ...
  actions: {
    setPermCodeList(codeList: string[]) {
      ...
      setAuthCache(AUHT_KEY, codeList);
    },
    ...
    async buildRoutesAction(id?: number | string): Promise<AppRouteRecordRaw[]> {
      ...
      if (permissionMode === PermissionModeEnum.ROLE) {
        const routeFilter = (route: AppRouteRecordRaw) => {
          ...
          return (permissions == undefined || permissions.length <= 0) ? true : permissions.includes(policy as string);
        };

...
export default {
  ...
  methods: {
    ...
    renderMenu: function (h, menuTree) {
      let this_ = this
      let menuArr = []
      this.filterMenuByPermission(menuTree)
    ...
    // 通过权限过滤路由
    filterMenuByPermission: function (menuTree) {
      menuTree.forEach(item => {
        if (item.children) {
          item.children.forEach(childrenItem => {
            if (this.hasPermission(childrenItem.meta.policy)) {
              childrenItem.meta.invisible = false;
            } else {
              childrenItem.meta.invisible = true
            }
          })
        } else {
          if (this.hasPermission(item.meta.policy)) {
            item.meta.invisible = false;
          } else {
            item.meta.invisible = true;
          }
        }
      })
    },
